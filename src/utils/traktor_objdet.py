...
from MOTobjdet import MOTSynthObjDet, MOT17ObjDet, MOTSynthFramesObjDet
    ...
    def __init__(self, path, mode, transform=None):
        ...
        if mode == 'train':
            for seq_dir in seq_dirs:
                seqs = [seq[:-4] for seq in os.listdir(os.path.join(self.path,seq_dir))]
                seqs.sort()
                dict[seq_dir] = seqs
                for seq in seqs:
                    config = ConfigParser()
...
if __name__ == '__main__':
    ...
    transforms = [Resize(IMAGE_RESOLUTION), Normalize(mean=MEAN, std=STD)]
    transforms_synth = transforms[:]
    transform = Compose(transforms)
    transform_synth = Compose(transforms_synth)
    ...
    elif args.dataset_class=='MOT17':
        print('MOT Synth selected')
        dataset = MOT17ObjDet(path=args.dataset, mode=args.mode, transform=transform)
    else:
        print('Unknown dataset class entered, please verify')
    print(f'Real dataset has {len(dataset)} frames.')

    print(f'Data: {args.dataset}')
    if args.synth_dataset is not None:
        print(f'Synthetic data: {args.synth_dataset}')
        if args.dataset_class in ['UAVDT','VisDrone']:
            synth_dataset = AirSimObjDet(path=args.synth_dataset,transform=transform_synth,mode=args.synthmode)
        if args.dataset_class in ['MOT17']:
            synth_dataset = MOTSynthFramesObjDet(path=args.synth_dataset,transform=transform_synth,mode=args.synthmode)
        dataset = ConcatDataset([dataset,synth_dataset])
        print(f'Synthetic dataset has {len(synth_dataset)} frames.')
    else:
        print('No synthetic dataset provided, working with real data only.')
    #dataset_test = UAVDTObjDet(path=args.dataset, mode='test')
    print(f'Combined dataset has {len(dataset)} frames.')
