...
if __name__=='__main__':
    ...
    for seq in seqs:
        ...
        #    pass
        annot_file = os.path.join(annot_dir, seq,'gt/gt.txt')
        ...
        frames_nm = os.listdir(os.path.join(vids_path,seq,'rgb'))
        frames_nm.sort()
        ...
        for frame_id in tqdm(frame_ids):
            #print(frame_id)
            frame_pth = os.path.join(vids_path, seq,'rgb', frames_nm[frame_id-1])
            frame = Image.open(frame_pth)
            frame = np.array(frame)
