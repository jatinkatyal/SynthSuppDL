...
if __name__=='__main__':
    ...
    for seq in seqs:
        ...
        #    pass
        annot_file = os.path.join(annot_dir, f'{seq}.txt')
        annot = pd.read_csv(annot_file, delimiter=',', header=None,
                            names=['frame_id', 'object_id', 'x', 'y', 'w', 'h', 'score', 'category', 'in_view', 'occlusinon'])
        annot['frame_id'] = annot['frame_id'] - 1
        annot = annot[(annot.category==4) | (annot.category==5) | (annot.category==6) | (annot.category==9)]
        #annot = annot[(annot[:, 7] == 4) | (annot[:, 7] == 5) | (annot[:, 7] == 6) | (annot[:, 7] == 9)]  # Filtering vehicles
